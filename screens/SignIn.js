...
const SignIn = ({ navigation, route }) => {
  ...
  const { updateMatchingList, setMatchingList } =
    useContext(MatchingListContext)
    ...
    fetch('https://pets-tinder.herokuapp.com/api/user/login', {
      ...
      .then(async (res) => {
        if (res.status === 1) {
          ...
          setMatchingList(res.user.matching_list)
          const matchingListId = res.user.matching_list.map((item) => item._id)
          socket.emit('join', {
            token: res.token,
